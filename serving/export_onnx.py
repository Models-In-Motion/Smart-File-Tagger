...
# Docker: ONNX_EXPORT_DIR=/models so the file lands next to mounted LightGBM pickles.
OUT_DIR = Path(os.environ.get("ONNX_EXPORT_DIR", "models"))
...
# ---------------------------------------------------------------------------
# Step 4–5 — Benchmark + verify (skipped in Docker for faster builds)
# ---------------------------------------------------------------------------
SKIP_BENCH = os.environ.get("SKIP_ONNX_BENCHMARK", "").lower() in ("1", "true", "yes")

if SKIP_BENCH:
    pytorch_avg = onnx_avg = speedup = 0.0
    print("\nStep 4–5 — Skipped (SKIP_ONNX_BENCHMARK set).")
else:
...
# ---------------------------------------------------------------------------
bench_lines = (
    f"PyTorch avg  : {pytorch_avg:.2f}ms\nONNX avg     : {onnx_avg:.2f}ms\nSpeedup      : {speedup:.2f}x\n"
    if not SKIP_BENCH
    else "Benchmark    : skipped\n"
)
print(f"""
{'='*50}
